...
L.Control.BrowserPrint = L.Control.extend({
	...
	_appendControlStyles: function (container) {
		...
		printControlStyleSheet.innerHTML += " .leaflet-print-overlay { width: 100%; height:auto; min-height: 100%; position: absolute; top: 0; background-color: white!important; left: 0; z-index: 1001; display: block!important; overflow-x: scroll !important; } ";
	...
	_setupManualPrintButton: function (map, origins, objects) {
		...
		manualPrintButton.style.position = "fixed";
		manualPrintButton.style.top = "20px";
		manualPrintButton.style.right = "20px";
		manualPrintButton.style.zIndex = '999';
		...
		var cancelPrintButton = document.createElement('button');
		cancelPrintButton.className = "leaflet-browser-print--manualMode-button";
		cancelPrintButton.innerHTML = "Cancel Print";
		cancelPrintButton.style.position = "fixed";
		cancelPrintButton.style.top = "20px";
		cancelPrintButton.style.right = "80px";
		cancelPrintButton.style.zIndex = '999';
		this.__overlay__.appendChild(cancelPrintButton);

		// var selfCancel = this;
		L.DomEvent.addListener(cancelPrintButton, 'click', function () {
